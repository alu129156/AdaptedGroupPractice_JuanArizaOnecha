...
import com.example.weatherapp_juanarizaonecha.adapter.CommentAdapter
import com.example.weatherapp_juanarizaonecha.databinding.ActivityCityPostedCommentsBinding
...
class CityPostedComments : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        view.comments.etChatUsername.setText(DataUtils.user.name)
        view.comments.etChatEmail.setText(DataUtils.user.email)
        ...
        if (DataUtils.user.name.isEmpty() && DataUtils.user.email.isEmpty()){
        ...
        view.sendComment.btnPostComment.setOnClickListener {
            if(haveEmptyUsernameOrEmail()) {
                ...
                val comment = Comment(
                    newMessageKey, DataUtils.user.name, DataUtils.user.email,
        ...
        view.comments.btnChange.setOnClickListener {
            ...
            if (isEditing) {
                ...
                DataUtils.user.name = view.comments.etChatUsername.text.toString()
                DataUtils.user.email = view.comments.etChatEmail.text.toString()
                view.comments.etChatUsername.isEnabled = false
                view.comments.etChatEmail.isEnabled = false

                //Saved user in shared preferences
                repository.save(DataUtils.user.name + ";" + DataUtils.user.email)
                ...
                commentAdapter.commentsList.forEach {
                    if(it.email == DataUtils.user.email) {
                ...
                repository.delete(DataUtils.user.name + ";" + DataUtils.user.email)
        ...
        dbReference.addChildEventListener(object : ChildEventListener {
            override fun onChildAdded(snapshot: DataSnapshot, previousChildName: String?) {
                ...
                if(commentAdapter.commentsList.none { it.id == comment!!.id }){
                    ...
                    if(comment!!.email == DataUtils.user.email) {
                        comment.isCurrentUser = true
                        commentAdapter.insertNewComment(comment)
                    } else {
    ...
    private fun haveEmptyUsernameOrEmail(): Boolean {
        return DataUtils.user.name.isEmpty() || DataUtils.user.email.isEmpty()
    }
